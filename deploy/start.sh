...
# Jev / vision stay OFF on the public copy unless the operator sets these (and provides the key as a secret).
if [ -n "$(ls demo/jev_cache/*.json 2>/dev/null | head -1)" ]; then export SDV_JEV_CACHE=demo/jev_cache; fi
